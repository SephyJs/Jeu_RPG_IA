...
def player_sheet_panel(state: GameState, on_change) -> None:
    ...
    ui.label(f"Nom: {char_name}").classes("font-semibold")
    if char_id:
        ui.label(f"ID: {char_id}").classes("text-xs opacity-60")

    if not ready:
        labels = [_MISSING_LABELS.get(k, k) for k in missing]
        ui.label("Creation en cours.").classes("text-sm text-amber-8")
        if labels:
            ui.label("Infos manquantes: " + ", ".join(labels)).classes("text-xs opacity-80")
    else:
    ...
    ui.separator()
    ui.label(f"Genre: {identity.get('gender', 'inconnu')}").classes("text-sm")
    ui.label(f"Origine: {family.get('origin', 'inconnue')}").classes("text-sm")
    if str(visual.get("courte") or "").strip():
        ui.label("Apparence: " + str(visual.get("courte"))).classes("text-sm")
    if str(sheet.get("char_persona") or "").strip():
        ui.label("Personnalite: " + str(sheet.get("char_persona"))).classes("text-sm")

    passives = lore.get("passives")
    if isinstance(passives, list) and passives:
        ui.label("Atouts:").classes("text-sm font-semibold")
        for p in passives[:6]:
            ...
            if eff:
                label += f": {eff}"
            ui.label(label).classes("text-xs opacity-90")

    goals = world_logic.get("goals")
    if isinstance(goals, list) and goals:
        ui.label("Objectifs: " + ", ".join(str(g) for g in goals[:4] if isinstance(g, str))).classes("text-xs opacity-80")
    ...
    ui.label(f"Points de competence: {_safe_int(getattr(state, 'skill_points', 0), 0)}").classes("text-sm")
    ui.label(
    ...
    if effective_stats:
        ...
        if bonus_parts:
            ui.label("Bonus equipement: " + " | ".join(bonus_parts[:8])).classes("text-xs opacity-80")

    equipment = sheet.get("equipment_runtime", {}) if isinstance(sheet.get("equipment_runtime"), dict) else {}
    if equipment:
        ui.label("Equipe:").classes("text-sm font-semibold")
        for slot in ("weapon", "armor", "accessory_1", "accessory_2"):
            ...
            ui.label(f"- {slot}: {item_name}").classes("text-xs opacity-90")

    ui.separator()
    with ui.row().classes("w-full items-center justify-between"):
        ui.label("Journal de progression").classes("font-semibold")
        ...
        ui.button("Vider le journal", on_click=_clear_progress_log).props("outline dense size=sm no-caps")

    logs = state.player_progress_log if isinstance(state.player_progress_log, list) else []
    if not logs:
        ui.label("Aucun gain enregistre pour le moment.").classes("text-xs opacity-70")
    else:
        for entry in list(reversed(logs[-20:])):
